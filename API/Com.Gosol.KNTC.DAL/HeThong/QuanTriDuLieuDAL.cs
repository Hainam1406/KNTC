...
namespace Com.Gosol.KNTC.DAL.HeThong
{
    ...
    public class QuanTriDuLieuDAL
    {
        ...
        public int RestoreData(string fileName, string filePath)
        {
            var backupPath = new SystemConfigDAL().GetByKey("BackUp_Path").ConfigValue.ToString();
            ...
            try
            {
                return restoreService.RestoreDatabase(fileName, backupPath);
            }
            catch (Exception ex)
            {
                return -1;
        ...
        {
            List<FileTextMOD> preData = (List<FileTextMOD>)ReadFileTXT(filePath).Data;
