                ...
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, System.Data.CommandType.StoredProcedure, GETBYNAME, parameters))
                {
                    while (dr.Read())
                    {
                        SystemConfig = new SystemConfigModel(
                            Utils.ConvertToInt32(dr["SystemConfigID"], 0),
                            Utils.ConvertToString(dr["ConfigKey"], string.Empty),
                            Utils.ConvertToString(dr["ConfigValue"], string.Empty),
                            Utils.ConvertToString(dr["Description"], string.Empty)
                            );
                        break;
                    }
                    dr.Close();
