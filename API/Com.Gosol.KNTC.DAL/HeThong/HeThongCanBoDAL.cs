            ...
            using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        NguoiDungID_Insert = Utils.ConvertToInt32(SQLHelper.ExecuteScalar(trans, System.Data.CommandType.StoredProcedure, @"v2_HeThong_NguoiDung_Insert", paramrs), 0);
        ...
        public int Update(HeThongCanBoModel HeThongCanBoModel, ref string Message, int? NguoiDungID)
        {
            ...
            using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    {
                        string MessageNew = null;
                        val = SQLHelper.ExecuteNonQuery(trans, System.Data.CommandType.StoredProcedure, @"v2_HeThong_CanBo_Update_New", parameters);
                        trans.Commit();
                        //if (val > 0 && !UserRole.CheckAdmin(NguoiDungID.Value))
                        //{
                        //    var mess = "";
