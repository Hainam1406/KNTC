...
namespace Com.Gosol.KNTC.DAL.XuLyDon
{
    public class QL_EmailDAL
    {
        ...
        public List<QL_EmailInfo> GetBySear(string key, int page, int pagesize)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "QL_Email_GetBySearch", parameters))
                {
                    while (dr.Read())
                    {
