...
{
    public class DTXuLyDAL
    {
        #region
        /* #region cac bien va store
         // StoreProcedure
         private const string GET_DTDA_TIEPNHAN = @"XuLyDon_GetDTDaTiepNhan_V2";
         ...
         private const string PARAM_TOTALROW = "@TotalRow";
         #endregion

         // Don thu da tiep nhan
         public BaseResultModel GetDTDaTiepNhan(DTXuLyParam p)
         {
             var Result = new BaseResultModel();
             List<DTXulyMOD> ListInfo = new List<DTXulyMOD>();
             ...
             SqlParameter[] parms = new SqlParameter[] {
                 ...
                 new SqlParameter(PARAM_TOTALROW,SqlDbType.Int)

             };

             parms[0].Value = p.CoQuanID;
             parms[1].Value = p.Keyword != null ? p.Keyword : "";
             parms[2].Value = p.LoaiKhieuToID != null ? p.LoaiKhieuToID : 0;
             parms[3].Value = p.TuNgay ?? Convert.DBNull;
             parms[4].Value = p.DenNgay ?? Convert.DBNull;
             parms[5].Value = p.PageNumber <= 1 ? 1 : (p.PageNumber - 1) * p.PageSize + 1; ;
             parms[6].Value = p.PageNumber * p.PageSize;
             parms[7].Value = p.CanBoID;
             parms[8].Direction = ParameterDirection.Output;

             *//*if (p.CanBoID == 0) parms[7].Value = DBNull.Value;
             if (p.CoQuanID == 0) parms[0].Value = DBNull.Value; 

             if (p.TuNgay == DateTime.MinValue) parms[3].Value = DBNull.Value;
             if (p.DenNgay == DateTime.MinValue) parms[4].Value = DBNull.Value;*//*
             //SqlParameter[] parms = GetPara_Search();
             //SetPara_Search(parms, info, docList);
             try
             {
                 using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, GET_DTDA_TIEPNHAN, parms))
                 {
                     while (dr.Read())
                     {
                         DTXulyMOD info = new DTXulyMOD();
                         ...
                         info.SuDungQuyTrinh = Utils.ConvertToBoolean(dr["SuDungQuyTrinh"], false);
                         info.StateName = Utils.GetString(dr["StateName"], String.Empty);
                         if (info.StateName == Constant.CV_TiepNhan)
                         {
                             if (info.SuDungQuyTrinh == true)
                             {
                                 info.StateName = "Chưa trình";
                             }
                             else
                             {
                                 info.StateName = "Chưa xử lý";
                             }
                         }
                         else
                         {
                             if (info.SuDungQuyTrinh == true)
                             {
                                 info.StateName = "Đã trình";
                             }
                             else
                             {
                                 if (info.StateName == Constant.LD_Phan_GiaiQuyet)
                                 {
                                     info.StateName = "Đã xử lý";
                                 }
                                 else
                                 {
                                     if (info.StateName == Constant.Ket_Thuc)
                                     {
                                         info.StateName = "Đã xử lý";
                                     }
                                     else
                                     {
                                         info.StateName = "Đã giao";
                                     }
                                 }
                             }
                     ...
                     dr.Close();
                     Result.Status = 1;
                     Result.Message = "Danh sách đơn thư đã tiếp nhận";
                     Result.Data = ListInfo;
                     Result.TotalRow = Utils.ConvertToInt32(parms[8].Value, 0);
                 }
             }
             catch(Exception ex)
             {
        ...
        private const string GET_BY_ID = @"XuLyDon_GetByID";
        ...
        private const string UPDATE_HUONGXULY = @"[XuLyDon_Update_HuongXuLy]";

        // delete don thu
        private const string DELETE_DONTHU = @"DonThu_Delete_Supper_DTTiepNhan";
        ...
        public void SetPara_Search(SqlParameter[] parms, QueryFilterInfo info)//, List<int> documentIDlist)
        {
            ...
            parms[2].Value = info.LoaiKhieuToID != 0 ? info.LoaiKhieuToID : 0;
            parms[3].Value = info.TuNgay ?? Convert.DBNull;
            parms[4].Value = info.DenNgay ?? Convert.DBNull;
            parms[5].Value = info.Start <= 1 ? 1 : (info.Start - 1) * info.End + 1;
            parms[6].Value = info.Start * info.End;
        ...
        {
            var Result = new BaseResultModel();
            List<DTXuLyInfo> listDTCanXL = new List<DTXuLyInfo>();
            //SqlParameter[] parms = GetPara_Search();
            SqlParameter[] parms = new SqlParameter[] {
                new SqlParameter(PARAM_COQUANID, SqlDbType.Int),
                new SqlParameter(PARAM_KEY, SqlDbType.NVarChar,50),
            ...
            parms[0].Value = info.CoQuanID;
            parms[1].Value = info.Keyword != null ? info.Keyword : "";
            parms[2].Value = info.LoaiKhieuToID != null ? info.LoaiKhieuToID : 0;
            parms[3].Value = info.TuNgay ?? Convert.DBNull;
            parms[4].Value = info.DenNgay ?? Convert.DBNull;
            parms[5].Value = info.PageNumber <= 1 ? 1 : (info.PageNumber - 1) * info.PageSize + 1;
            parms[6].Value = info.PageNumber * info.PageSize;
            parms[7].Value = info.CanBoID;
            try
            {
        ...
        public IList<DTXuLyInfo> DTCanPhanXL_LanhDao(QueryFilterInfo info, List<int> docList)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, DTCANPHANXL_LANHDAO, parms))
                {
                    while (dr.Read())
                    {
        ...
        public IList<DTXuLyInfo> DTCanPhanXL_TruongPhong(QueryFilterInfo info, List<int> docList)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, DTCANPHANXL_TRUONGPHONG, parms))
                {
                    while (dr.Read())
                    {
                        DTXuLyInfo Info = GetDataDTPhanXuLy(dr);
        ...
        public BaseResultModel GetDTDaTiepNhan(DTXuLyParam p, bool SuDungQuyTrinh)
        {
            ...
            /*(p.CanBoID == 0) parms[7].Value = DBNull.Value;
             if (p.CoQuanID == 0) parms[0].Value = DBNull.Value;
        ...
        /*public IList<DTXuLyInfo> GetDTDaTiepNhan(QueryFilterInfo info, int canBoID)
        {
            IList<DTXuLyInfo> ListInfo = new List<DTXuLyInfo>();
        ...
        public IList<DTXuLyInfo> GetSoTiepNhanGianTiep_BTDTinh(QueryFilterInfo info)
        {
            IList<DTXuLyInfo> ListInfo = new List<DTXuLyInfo>();
        ...
        public IList<DTXuLyInfo> DTDuyetKQXL_LanhDao(QueryFilterInfo info, List<int> docList)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, GET_DTDUYETXL_LANHDAO_NEW, parms))
                {
                    while (dr.Read())
                    {
        ...
        public IList<DTXuLyInfo> DTDuyetKQXL_LanhDao_GetAllByCoQuanID(QueryFilterInfo info, List<int> docList)
        {
            ...
            try
            {
                using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, XULYDON_GETDTDUYETXL_LANHDAO_GETALL_BYCOQUANID_NEW, parms))
                {
        ...
        public BaseResultModel XoaDonThu(ParamXoa p)
        {
            var Result = new BaseResultModel();
            int qr = 0;
            SqlParameter[] parameters = new SqlParameter[]
            {
               ...
               new SqlParameter("@pDoiTuongBiKNID", SqlDbType.Int),
               new SqlParameter("@pNhomKNID", SqlDbType.Int),
            };
            parameters[0].Value = p.DonThuID;
            parameters[1].Value = p.XuLyDonID;
            parameters[2].Value = p.pDoiTuongBiKNID ?? Convert.DBNull;
            parameters[3].Value = p.pNhomKNID;
            ...
            using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        //var qr = SQLHelper.ExecuteNonQuery(trans, System.Data.CommandType.StoredProcedure, DELETE_DONTHU, parameters);
                        qr = SQLHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, DELETE_DONTHU, parameters);
        ...
        public BaseResultModel Update_HuongXuLy(DTXuLyInfo item)
        {
            ...
            try
            {
                ...
                parameters[0].Value = item.XuLyDonID;
                parameters[1].Value = item.HuongGiaiQuyetID ?? Convert.DBNull;
                parameters[2].Value = item.NoiDungHuongDan ?? Convert.DBNull;
                parameters[3].Value = item.CanBoKyID ?? Convert.DBNull;
                using (SqlConnection conn = new SqlConnection(SQLHelper.appConnectionStrings))
                {
                    conn.Open();
                    using (SqlTransaction trans = conn.BeginTransaction())
                    {
                        try
                        {
                            Result.Status = SQLHelper.ExecuteNonQuery(trans, CommandType.StoredProcedure, UPDATE_HUONGXULY, parameters);
                            trans.Commit();
                            Result.Message = "Thêm hướng xử lý thành công!";
