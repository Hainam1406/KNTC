    ...
    public class XLD01DAL
    {
        public List<TableData> XLD01(List<int> listCapChonBaoCao, string ContentRootPath, int RoleID, int CapID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, int HuyenDangNhapID, DateTime startDate, DateTime endDate)
        {
            ...
            foreach (DataRow dro in dt.Rows)
            {
                ...
                int col10 = Utils.ConvertToInt32(dro["Col10Data"], 0);
                int col11 = Utils.ConvertToInt32(dro["Col11Data"], 0);
                int col12 = Utils.ConvertToInt32(dro["Col12Data"], 0);
                int col13 = Utils.ConvertToInt32(dro["Col13Data"], 0);
                int col19 = Utils.ConvertToInt32(dro["Col19Data"], 0);
                int col20 = Utils.ConvertToInt32(dro["Col20Data"], 0);
                int col21 = Utils.ConvertToInt32(dro["Col21Data"], 0);
                int col23 = Utils.ConvertToInt32(dro["Col23Data"], 0);
                int col24 = Utils.ConvertToInt32(dro["Col24Data"], 0);
                int col25 = Utils.ConvertToInt32(dro["Col25Data"], 0);

                //int col1 = col2 + col3 + col4 + col5;
                int col9 = col11 + col12 + col13;
                int col18 = col19 + col20 + col21;
                int col22 = col23 + col24 + col25;
                ...
                RowItem RowItem13 = new RowItem(13, Utils.AddCommas(dro["Col12Data"].ToString()), dro["CoQuanID"].ToString(), dro["CapID"].ToString(), null, "text-align: center;" + Css, ref DataArr);
        ...
        public void CalculateBaoCaoCapTinh(string ContentRootPath, int RoleID, int CoQuanDangNhapID, int CanBoDangNhapID, int TinhDangNhapID, ref List<BaoCao2bInfo> resultList, DateTime startDate, DateTime endDate, List<CapInfo> capList, bool flag, int tinhID, ref DataTable dt)
        {
            ...
            if (capList.Count > 0)
            {
                ...
                if (cqList.Count > 0)
                {
                    ...
                    try
                    {
                        using (SqlDataReader dr = SQLHelper.ExecuteReader(SQLHelper.appConnectionStrings, CommandType.StoredProcedure, "v2_BaoCaoXLD01", parm))
                        {
                            dt.Load(dr);
                        }
                    }
