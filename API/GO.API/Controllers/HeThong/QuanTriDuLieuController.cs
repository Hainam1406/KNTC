    ...
    public class QuanTriDuLieuController : BaseApiController
    {
        ...
        public IActionResult BackupDatabase(string fileName)
        {
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var filePath = _host.ContentRootPath + "/Upload/GhiLogQuanTriDuLieu.txt";
                        ...
                        string NguoiThucHien = Utils.ConvertToString(User.Claims.FirstOrDefault(x => x.Type == ("TenCanBo")).Value, string.Empty); 
                        _QuanTriDuLieuBUS.WirteFile(ThaoTac, ThoiGian, NguoiThucHien, filePath);
        ...
        public IActionResult RestoreDatabase(string fileName)
        {
            try
            {
                var filePath = _host.ContentRootPath + "/Upload/GhiLogQuanTriDuLieu.txt";
                int val = 0;
                val = _QuanTriDuLieuBUS.RestoreDatabase(fileName, _host.ContentRootPath);
                if (val == -1)
                {
                    base.Message = ConstantLogMessage.API_Error_System;
                    return base.GetActionResult();
        ...
        public IActionResult ReadFileTXT()
        {
            ...
            {
                var filePath = _host.ContentRootPath + "/Upload/GhiLogQuanTriDuLieu.txt";
                var Result = _QuanTriDuLieuBUS.ReadFileTXT(filePath);
                if (Result.Status < 1)
                {
                    Status = 0;
        ...
        public IActionResult TimKIem(string? Keyword)
        {
            ...
            {
                var filePath = _host.ContentRootPath + "/Upload/GhiLogQuanTriDuLieu.txt";
                var Result = _QuanTriDuLieuBUS.TimKiem(Keyword, filePath);
                if (Result != null) return Ok(Result);
                else return NotFound();
            }
            catch (Exception ex)
